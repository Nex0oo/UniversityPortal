...
$success_msg = '';
$error_msg = '';
if (isset($_POST['id']) && isset($_POST['nome']) && isset($_POST['cognome']) && isset($_POST['username']) && isset($_POST['pw'])) {
    if (!empty($_POST['id'])) {
        $id = $_POST['id'];
    } else {
        $error_msg = 'Campo id vuoto';
    }
    ...
    if (empty($error_msg)) {
        ...
        $sql = "INSERT INTO unidb.docente VALUES ($1, $2, $3, $4, $5)";
        ...
        $result = pg_prepare($db, "aggiungi_docente", $sql);
        $result = pg_execute($db, "aggiungi_docente", $params);
        if ($result) {
            $success_msg = 'Docente aggiunto correttamente';
        } else {
  ...
  <body>
    <nav class="navbar navbar-expand-lg bg-body-tertiary bg-dark border-bottom border-bottom-dark" data-bs-theme="dark"">
      ...
      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        <div class="row mt-5">
            <h4>Inserisci le informazioni del nuovo docente:</h4>
        </div>
        <div class="row mt-4">
            <div class="col">
            <input type="text" class="form-control-sm" placeholder="Id docente" aria-label="id" name="id">
            </div>
            <div class="col">
